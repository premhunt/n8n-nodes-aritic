...
export class Aritic implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Aritic',
		name: 'aritic',
		icon: 'file:aritic.svg',
		group: ['output'],
		version: 1,
		subtitle: '={{$parameter["operation"] + ": " + $parameter["resource"]}}',
		description: 'Consume Aritic API',
		defaults: {
			name: 'Aritic',
			color: '#52619b',
		},
	...
	methods = {
		loadOptions: {
			...
			async getCompanies(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				...
				const companies = await ariticApiRequestAllItems.call(this, 'companies', 'GET', '/companies');
			...
			async getTags(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				const returnData: INodePropertyOptions[] = [];
				const tags = await ariticApiRequestAllItems.call(this, 'tags', 'GET', '/tags');
			...
			async getStages(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				const returnData: INodePropertyOptions[] = [];
				const stages = await ariticApiRequestAllItems.call(this, 'stages', 'GET', '/stages');
			...
			async getCompanyFields(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				const returnData: INodePropertyOptions[] = [];
				const fields = await ariticApiRequestAllItems.call(this, 'fields', 'GET', '/fields/company');
			...
			async getContactFields(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				const returnData: INodePropertyOptions[] = [];
				const fields = await ariticApiRequestAllItems.call(this, 'fields', 'GET', '/fields/contact');
				for (const field of fields) {
					returnData.push({
						name: field.label,
						value: field.alias,
			...
			async getSegments(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				const returnData: INodePropertyOptions[] = [];
				const segments = await ariticApiRequestAllItems.call(this, 'lists', 'GET', '/segments');
			...
			async getCampaigns(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				const returnData: INodePropertyOptions[] = [];
				const campaings = await ariticApiRequestAllItems.call(this, 'campaigns', 'GET', '/campaigns');
			...
			async getEmails(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				const returnData: INodePropertyOptions[] = [];
				const emails = await ariticApiRequestAllItems.call(this, 'emails', 'GET', '/emails');
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < length; i++) {
			...
			try {
				...
				if (resource === 'company') {
					//https://developers.aritic.com/#create-company
					if (operation === 'create') {
						...
						responseData = await ariticApiRequest.call(this, 'POST', '/companies/new', body);
						responseData = responseData.company;
						if (simple === true) {
							responseData = responseData.fields.all;
						}
					}
					//https://developers.aritic.com/#edit-company
					if (operation === 'update') {
						...
						responseData = await ariticApiRequest.call(this, 'PATCH', `/companies/${companyId}/edit`, body);
						responseData = responseData.company;
						if (simple === true) {
							responseData = responseData.fields.all;
						}
					}
					//https://developers.aritic.com/#get-company
					if (operation === 'get') {
						const companyId = this.getNodeParameter('companyId', i) as string;
						const simple = this.getNodeParameter('simple', i) as boolean;
						responseData = await ariticApiRequest.call(this, 'GET', `/companies/${companyId}`);
						responseData = responseData.company;
						if (simple === true) {
							responseData = responseData.fields.all;
						}
					}
					//https://developers.aritic.com/#list-contact-companies
					if (operation === 'getAll') {
						...
						if (returnAll === true) {
							responseData = await ariticApiRequestAllItems.call(this, 'companies', 'GET', '/companies', {}, qs);
						} else {
							qs.limit = this.getNodeParameter('limit', i) as number;
							qs.start = 0;
							responseData = await ariticApiRequest.call(this, 'GET', '/companies', {}, qs);
					...
					//https://developers.aritic.com/#delete-company
					if (operation === 'delete') {
						const simple = this.getNodeParameter('simple', i) as boolean;
						const companyId = this.getNodeParameter('companyId', i) as string;
						responseData = await ariticApiRequest.call(this, 'DELETE', `/companies/${companyId}/delete`);
						responseData = responseData.company;
						if (simple === true) {
							responseData = responseData.fields.all;
						}
				...
				if (resource === 'contact') {
					//https://developers.aritic.com/?php#create-contact
					if (operation === 'create') {
						...
						responseData = await ariticApiRequest.call(this, 'POST', '/contacts/new', body);
						responseData = [responseData.contact];
						if (options.rawData === false) {
							responseData = responseData.map(item => item.fields.all);
						}
					}
					//https://developers.aritic.com/?php#edit-contact
					if (operation === 'update') {
						const options = this.getNodeParameter('options', i) as IDataObject;
						...
						responseData = await ariticApiRequest.call(this, 'PATCH', `/contacts/${contactId}/edit`, body);
						responseData = [responseData.contact];
						if (options.rawData === false) {
							responseData = responseData.map(item => item.fields.all);
						}
					}
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/?php#get-contact
					if (operation === 'get') {
						const options = this.getNodeParameter('options', i) as IDataObject;
						const contactId = this.getNodeParameter('contactId', i) as string;
						responseData = await ariticApiRequest.call(this, 'GET', `/contacts/${contactId}`);
						responseData = [responseData.contact];
						if (options.rawData === false) {
							responseData = responseData.map(item => item.fields.all);
						}
					}
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/?php#list-contacts
					if (operation === 'getAll') {
						const returnAll = this.getNodeParameter('returnAll', i) as boolean;
						const options = this.getNodeParameter('options', i) as IDataObject;
						...
						if (returnAll === true) {
							responseData = await ariticApiRequestAllItems.call(this, 'contacts', 'GET', '/contacts', {}, qs);
						} else {
							qs.limit = this.getNodeParameter('limit', i) as number;
							qs.start = 0;
							responseData = await ariticApiRequest.call(this, 'GET', '/contacts', {}, qs);
							if (responseData.errors) {
								throw new NodeApiError(this.getNode(), responseData);
							}
					...
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/?php#delete-contact
					if (operation === 'delete') {
						const options = this.getNodeParameter('options', i) as IDataObject;
						const contactId = this.getNodeParameter('contactId', i) as string;
						responseData = await ariticApiRequest.call(this, 'DELETE', `/contacts/${contactId}/delete`);
						responseData = [responseData.contact];
						if (options.rawData === false) {
							responseData = responseData.map(item => item.fields.all);
						}
					}
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/#send-email-to-contact
					if (operation === 'sendEmail') {
						const contactId = this.getNodeParameter('contactId', i) as string;
						const campaignEmailId = this.getNodeParameter('campaignEmailId', i) as string;
						responseData = await ariticApiRequest.call(this, 'POST', `/emails/${campaignEmailId}/contact/${contactId}/send`);
					}
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/#add-do-not-contact
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/#remove-from-do-not-contact
					if (operation === 'editDoNotContactList') {
						...
						responseData = await ariticApiRequest.call(this, 'POST', `/contacts/${contactId}/dnc/${channel}/${action}`, body);
						responseData = responseData.contact;
					}

					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/#add-points
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/#subtract-points
					if (operation === 'editContactPoint') {
						const contactId = this.getNodeParameter('contactId', i) as string;
						const action = this.getNodeParameter('action', i) as string;
						const points = this.getNodeParameter('points', i) as string;
						const path = (action === 'add') ? 'plus' : 'minus';
						responseData = await ariticApiRequest.call(this, 'POST', `/contacts/${contactId}/points/${path}/${points}`);
					}
				}

				if (resource === 'contactSegment') {
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/?php#add-contact-to-a-segment
					if (operation === 'add') {
						const contactId = this.getNodeParameter('contactId', i) as string;
						const segmentId = this.getNodeParameter('segmentId', i) as string;
						responseData = await ariticApiRequest.call(this, 'POST', `/segments/${segmentId}/contact/${contactId}/add`);
					}
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/#remove-contact-from-a-segment
					if (operation === 'remove') {
						const contactId = this.getNodeParameter('contactId', i) as string;
						const segmentId = this.getNodeParameter('segmentId', i) as string;
						responseData = await ariticApiRequest.call(this, 'POST', `/segments/${segmentId}/contact/${contactId}/remove`);
					}
				}

				if (resource === 'campaignContact') {
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/#add-contact-to-a-campaign
					if (operation === 'add') {
						const contactId = this.getNodeParameter('contactId', i) as string;
						const campaignId = this.getNodeParameter('campaignId', i) as string;
						responseData = await ariticApiRequest.call(this, 'POST', `/campaigns/${campaignId}/contact/${contactId}/add`);
					}
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/#remove-contact-from-a-campaign
					if (operation === 'remove') {
						const contactId = this.getNodeParameter('contactId', i) as string;
						const campaignId = this.getNodeParameter('campaignId', i) as string;
						responseData = await ariticApiRequest.call(this, 'POST', `/campaigns/${campaignId}/contact/${contactId}/remove`);
					}
				}

				if (resource === 'segmentEmail') {
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/#send-email-to-segment
					if (operation === 'send') {
						const segmentEmailId = this.getNodeParameter('segmentEmailId', i) as string;
						responseData = await ariticApiRequest.call(this, 'POST', `/emails/${segmentEmailId}/send`);
					}
				}

				if (resource === 'companyContact') {
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/#add-contact-to-a-company
					if (operation === 'add') {
						const contactId = this.getNodeParameter('contactId', i) as string;
						const companyId = this.getNodeParameter('companyId', i) as string;
						responseData = await ariticApiRequest.call(this, 'POST', `/companies/${companyId}/contact/${contactId}/add`, {});
						// responseData = responseData.company;
						// if (simple === true) {
						// 	responseData = responseData.fields.all;
						// }
					}
					//https://developers.aritic.com/aritic-pinpoint/#aritic-pinpoint-api-doc/#remove-contact-from-a-company
					if (operation === 'remove') {
						const contactId = this.getNodeParameter('contactId', i) as string;
						const companyId = this.getNodeParameter('companyId', i) as string;
						responseData = await ariticApiRequest.call(this, 'POST', `/companies/${companyId}/contact/${contactId}/remove`, {});
						// responseData = responseData.company;
						// if (simple === true) {
						// 	responseData = responseData.fields.all;
						// }
