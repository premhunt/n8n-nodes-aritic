...
export class AriticTrigger implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Aritic Trigger',
		name: 'ariticTrigger',
		icon: 'file:aritic.svg',
		group: ['trigger'],
		version: 1,
		description: 'Handle Aritic events via webhooks',
		defaults: {
			name: 'Aritic Trigger',
		},
		inputs: [],
	...
	methods = {
		loadOptions: {
			...
			async getEvents(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				...
				const { triggers } = await ariticApiRequest.call(this, 'GET', '/hooks/triggers');
	...
	webhookMethods = {
		default: {
			async checkExists(this: IHookFunctions): Promise<boolean> {
				...
				try {
					await ariticApiRequest.call(this, 'GET', endpoint, {});
				} catch (error) {
					return false;
				}
				return true;
			...
			async create(this: IHookFunctions): Promise<boolean> {
				...
				const { hook } = await ariticApiRequest.call(this, 'POST', '/hooks/new', body);
			...
			async delete(this: IHookFunctions): Promise<boolean> {
				const webhookData = this.getWorkflowStaticData('node');
				try {
					await ariticApiRequest.call(this, 'DELETE', `/hooks/${webhookData.webhookId}/delete`);
				} catch (error) {
					return false;
				}
